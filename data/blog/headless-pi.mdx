...
I'm using latest Raspberry Pi Bookworm 64-bit OS. Before installing, customize the OS by set the hostname, Wifi SSID & Password, enabling SSH etc. For more info, read
...
than X11 used in previous OS generation.
...
I notice that, when I enabling VNC **after** I do the [set default boot mode to CLI](#set-default-boot-mode-to-cli), it will create VNC config based 
on X11 backend, not wayland. This will cause the VNC viewer to show blank screen.
</Admonition>
