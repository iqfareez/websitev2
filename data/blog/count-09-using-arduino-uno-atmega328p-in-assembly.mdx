...
The lab task assigned to us was to count from 00-99 using 7 Segment Display, the program must be written in [Assembly](https://youtu.be/4gwYkEK0gOk). Fortunately, the 7 segment decoder was not enough for
everyone in the lab, so the tasks are simplified **to count from 0-9 only, and repeats**.

As someone who familiar with high-level programming language like Java, Python, etc, coding in assembly is infuriating 😂. The sweet side is about Assembly allows us to 'talk' directly to the bare metal, removing
the unnecessary abstraction, solve performance issues, code like a chad and understand how the hardware works.
